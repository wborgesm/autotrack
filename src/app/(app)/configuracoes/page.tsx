...
export default function ConfiguracoesPage() {
  ...
  const [oficina, setOficina] = useState({ nome: "", telefone: "", email: "", endereco: "", logo: "", latitude: "", longitude: "", raioPermitido: "100", tipoOficina: "AMBOS" });
  ...
  useEffect(() => {
    ...
    fetch("/api/configuracoes").then(r => r.json()).then(d => {
      setOficina(prev => ({ ...prev, ...d.oficina }));
      setAddons(d.addons || {});
    }).catch(() => {});
  }, [session]);
          ...
          <Card className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><Label>Nome</Label><Input className="bg-gray-100 dark:bg-gray-700" value={oficina.nome} onChange={e => setOficina({...oficina, nome: e.target.value})} /></div>
                <div><Label>Telefone</Label><Input className="bg-gray-100 dark:bg-gray-700" value={oficina.telefone} onChange={e => setOficina({...oficina, telefone: e.target.value})} /></div>
                <div><Label>Email</Label><Input className="bg-gray-100 dark:bg-gray-700" value={oficina.email} onChange={e => setOficina({...oficina, email: e.target.value})} /></div>
                <div><Label>Morada</Label><Input className="bg-gray-100 dark:bg-gray-700" value={oficina.endereco} onChange={e => setOficina({...oficina, endereco: e.target.value})} /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>Tipo de Oficina</Label>
                  <Select value={oficina.tipoOficina} onValueChange={v => setOficina({...oficina, tipoOficina: v})}>
                    <SelectTrigger className="bg-gray-100 dark:bg-gray-700"><SelectValue /></SelectTrigger>
                    <SelectContent className="bg-white dark:bg-gray-800">
                      <SelectItem value="CARROS">🚗 Carros</SelectItem>
                      <SelectItem value="MOTOS">🏍️ Motos</SelectItem>
              ...
              <div className="border-t pt-4 mt-4">
                <p className="font-semibold mb-2">📍 Localização para Ponto</p>
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div><Label>Raio (m)</Label><Input className="bg-gray-100 dark:bg-gray-700" type="number" value={oficina.raioPermitido} onChange={e => setOficina({...oficina, raioPermitido: e.target.value})} /></div>
                </div>
              </div>

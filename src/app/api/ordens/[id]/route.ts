...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!checkApiPermissao(session.user.nivel, "ordens")) return NextResponse.json({ error: "Permissão negada" }, { status: 403 });

  const ordem = await prisma.ordemServico.findFirst({
    where: { id: params.id, tenantId },
    include: { cliente: true, veiculo: true, tecnico: true, itens: { include: { servico: true } }, itensPeca: { include: { peca: true } }, historico: { orderBy: { createdAt: "desc" } }, lancamentos: true },
  });
  if (!ordem) return NextResponse.json({ error: "Ordem não encontrada" }, { status: 404 });
  if (!podeGerenciarOrdem(session.user.nivel, session.user.id, ordem.tecnicoId, ordem.usuarioId)) return NextResponse.json({ error: "Permissão negada" }, { status: 403 });
...
const patchSchema = z.object({
  ...
  laudoTecnico: z.string().optional(), observacoes: z.string().optional(), tecnicoId: z.string().optional(), kmSaida: z.number().int().optional(), dataEntrega: z.string().datetime().optional(),
  totalMaoObra: z.number().optional(), totalPecas: z.number().optional(), desconto: z.number().optional(), total: z.number().optional(), pago: z.boolean().optional(),
});
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!ordemAtual) return NextResponse.json({ error: "Ordem não encontrada" }, { status: 404 });
  if (!podeGerenciarOrdem(session.user.nivel, session.user.id, ordemAtual.tecnicoId, ordemAtual.usuarioId)) return NextResponse.json({ error: "Permissão negada" }, { status: 403 });
    ...
    const ordem = await prisma.$transaction(async (tx: any) => {
      ...
      if (novoStatus && novoStatus !== statusAnterior) {
        await tx.historicoOrdem.create({
          data: { ordemId: params.id, status: novoStatus, usuarioNome: session.user.name || session.user.email, observacao: validated.observacoes || `Status alterado de ${statusAnterior} para ${novoStatus}` },
        });
      }
      return updated;
    });
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const ordemAtual = await prisma.ordemServico.findFirst({ where: { id: params.id, tenantId } });
  if (!ordemAtual) return NextResponse.json({ error: "Ordem não encontrada" }, { status: 404 });
  if (!podeGerenciarOrdem(session.user.nivel, session.user.id, ordemAtual.tecnicoId, ordemAtual.usuarioId)) return NextResponse.json({ error: "Permissão negada" }, { status: 403 });

  const ordem = await prisma.$transaction(async (tx: any) => {
    const updated = await tx.ordemServico.update({ where: { id: params.id }, data: { status: "CANCELADA" } });
    await tx.historicoOrdem.create({ data: { ordemId: params.id, status: "CANCELADA", usuarioNome: session.user.name || session.user.email, observacao: "Ordem cancelada pelo usuário" } });
    return updated;
  });
  return NextResponse.json(ordem);
}
